    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            <div class="bg-white rounded-xl border border-gray-200 px-5 py-4">
                ...
                <p class="mt-1 text-2xl font-bold text-gray-900" id="stat-total">{{ $total }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 px-5 py-4">
                <p class="text-xs font-medium text-green-600 uppercase tracking-wide">Published</p>
                <p class="mt-1 text-2xl font-bold text-gray-900" id="stat-published">{{ $published }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 px-5 py-4">
                <p class="text-xs font-medium text-amber-600 uppercase tracking-wide">Drafts</p>
                <p class="mt-1 text-2xl font-bold text-gray-900" id="stat-drafts">{{ $drafts }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 px-5 py-4">
                <p class="text-xs font-medium text-gray-400 uppercase tracking-wide">Archived</p>
                <p class="mt-1 text-2xl font-bold text-gray-900" id="stat-archived">{{ $archived }}</p>
            </div>
        </div>

        {{-- Table card with search at top of table --}}
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            {{-- Search bar - inside table card, above the table --}}
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100">
                    ...
                    <tbody id="pages-table-body">
                        @include('admin.seo_pages.partials.pages-table', ['pages' => $pages])
                    </tbody>
                </dTable
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch() {
        ...
        fetch(`{{ route('admin.seo-pages.index') }}?search=${encodeURIComponent(query)}`, {
            headers: {
